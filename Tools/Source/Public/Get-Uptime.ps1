Function Get-Uptime {
    ...
    Process {
        ...
        {   Write-Verbose "Checking Host: $Computer"
            Try {
                $LastBoot = Get-WmiObject -ComputerName $Computer -Query "SELECT LastBootUpTime FROM Win32_OperatingSystem" -ErrorAction Stop
            }
            Catch {
            ...
            $Result | Select Name,LastBootTime,RebootSince
        }
    }
}